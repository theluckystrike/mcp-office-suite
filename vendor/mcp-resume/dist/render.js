...
export function wordsPerPage(style) {
...
export function keywordReport(corpus, keywords) {
    const matched = [];
    const missing = [];
 ...
 */
export function scoreBullets(experience, keywords) {
    const out = [];
...
export function trimToPages(experience, keywords, fixedWords, maxPages, perPage) {
    ...
    for (const b of ranked) {
        if (used + b.words > budget && kept.length)
            continue;
        kept.push(b);
        used += b.words;
    }
...
export function fixedWordCount(p, targetRole) {
    let n = countWords(p.name) + countWords(contactLine(p)) + countWords(targetRole ?? "");
    n += countWords(p.summary ?? "") + 2;
    for (const s of p.skills ?? [])
        n += countWords(s) + 1;
    for (const e of p.experience)
        n += countWords(e.company) + countWords(e.title) + countWords(dateRange(e.start, e.end)) + 2;
    for (const e of p.education)
        n += countWords(e.school) + countWords(e.degree) + countWords(dateRange(e.start, e.end)) + 2;
...
export function renderResume(p, o) {
    ...
    const hl = (s) => (keywords.length ? highlight(s, keywords) : s);
    const blocks = [];
    const contact = contactLine(p);
    if (contact)
        blocks.push({ type: "para", text: contact });
    if (o.targetRole)
        blocks.push({ type: "para", text: `**${o.targetRole}**` });
    if (p.summary) {
    ...
    if (p.experience.length) {
        ...
        p.experience.forEach((e, ei) => {
            ...
            if (items.length)
                blocks.push({ type: "bullets", items, ordered: false, levels: items.map(() => 0) });
        });
    }
...
export function profileCorpus(p) {
    const parts = [p.summary ?? "", ...(p.skills ?? []), ...(p.certifications ?? []), ...(p.languages ?? [])];
    for (const e of p.experience)
