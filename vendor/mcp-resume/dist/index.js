...
import { buildDocx, letterhead, readDocx, stripInvalidXml, toHtml } from "@theluckystrike/mcp-docx/lib";
import { z } from "zod";
import { addLetter, dataDir, getProfile, lettersInMonth, normalizeVariant, setProfile, sortExperienceNewestFirst, variantNames, } from "./profile.js";
import { blocksToProfile } from "./read.js";
import { buildLetter, unsourcedNumbers } from "./letter.js";
import { blocksToMarkdown, renderResume } from "./render.js";
import { analyseGap } from "./tailor.js";
import { VERSION } from "./version.js";
const FREE_LETTERS_PER_MONTH = 3;
const FREE_JD_CHARS = 2000;
const FREE_STYLE = "modern";
...
function expandPath(p) {
    if (URL_SCHEME_RE.test(p)) {
        throw new Error(`"${p}" is a URL, not a file path; this tool reads local files. On the hosted route, ` +
...
function clean(s) { return stripInvalidXml(s).text; }
function cleanBlocks(blocks) {
    return blocks.map((b) => {
        if (b.type === "bullets")
            return { ...b, items: b.items.map(clean) };
        if (b.type === "table")
            return { ...b, headers: b.headers.map(clean), rows: b.rows.map((r) => r.map(clean)) };
        return { ...b, text: clean(b.text) };
    });
}
function proOnly(what) { return `${what} is a Pro feature.\n\n${gate.upgradeText(what)}`; }
/**
 ...
 */
function bulletChanges(before, after) {
    if (!before)
        return "";
    const key = (e) => `${e.company}|${e.title}`;
    ...
    for (const e of after.experience) {
        ...
        if (!prev) {
            added.push(...e.bullets.map((b) => `${e.company}: ${b}`));
            continue;
        }
        const seen = new Set(prev);
        for (const b of e.bullets)
            if (!seen.has(b))
                changed.push(`${e.company}: "${b}"`);
        const now = new Set(e.bullets);
        for (const b of prev)
            if (!now.has(b))
                removed.push(`${e.company}: "${b}"`);
    }
    if (!changed.length && !added.length && !removed.length)
        return "\n\nNo stored bullet changed.";
    const parts = [];
    if (changed.length)
        parts.push(`${changed.length} bullet(s) now read differently from what was stored: ${changed.join("; ")}`);
    if (removed.length)
        parts.push(`${removed.length} stored bullet(s) are gone: ${removed.join("; ")}`);
    if (added.length)
        parts.push(`${added.length} bullet(s) are on roles that were not stored before: ${added.join("; ")}`);
...
function requireProfile(variant) {
...
const server = new McpServer({ name: "mcp-resume", version: VERSION }, { capabilities: { tools: {}, resources: {}, prompts: {} } });
/* ---------------------------------------------------------------- schemas */
...
server.registerTool("profile_set", {
    ...
    try {
        ...
        if (wantsVariant && !gate.isPro())
            return fail(proOnly("Profile variants"));
        ...
        if (existing && !a.merge && !a.replace) {
            return fail(`a profile is already stored under variant "${normalizeVariant(a.variant)}" ` +
        ...
        if (!name) {
            return fail(`no name: pass name, or set one once in the shared business profile (the invoice server's business_set, field name) and every server in this suite uses it. Nothing was stored.`);
        }
        const fromProfile = [];
        if (!a.name && !base?.name && shared.name)
            fromProfile.push("name");
        const p = {
            name,
            ...
            experience: sortExperienceNewestFirst((a.experience.length ? a.experience : base?.experience ?? [])),
            education: (a.education.length ? a.education : base?.education ?? []),
        ...
        if (!a.phone && !base?.phone && shared.phone)
            fromProfile.push("phone");
        if (!a.email && !base?.email && shared.email)
            fromProfile.push("email");
...
server.registerTool("resume_create", {
    ...
    try {
        ...
        if (typeof p === "string")
            return fail(p);
        const style = a.style;
        if (style !== FREE_STYLE && !gate.isPro())
            return fail(proOnly(`The "${style}" resume style`));
...
server.registerTool("resume_to_html", {
    ...
    try {
        ...
        if (typeof p === "string")
            return fail(p);
        const r = renderResume(p, { style: "modern", targetRole: a.target_role, maxPages: a.max_pages });
        const blocks = [{ type: "heading", level: 1, text: p.name }, ...r.blocks];
...
server.registerTool("resume_read", {
    ...
    try {
        ...
        if (!/\.docx$/i.test(file))
            return fail(`${file} is not a .docx file. Legacy .doc and .rtf are not readable here.`);
        const blocks = readDocx(readFileSync(file));
        const r = blocksToProfile(blocks);
        if (a.save) {
            if (a.variant && normalizeVariant(a.variant) !== "default" && !gate.isPro())
                return fail(proOnly("Profile variants"));
            if (!r.profile.name)
                return fail("no name could be read from the document, so nothing was saved. Call again with save: false, then pass the corrected fields to profile_set.");
...
server.registerTool("cover_letter_create", {
    ...
    try {
        ...
        const letter = buildLetter(p, {
            ...
            job_description: a.job_description, tone: a.tone, highlights: a.highlights,
        });
        ...
        if (bad.length)
            return fail(`refusing to write the letter: it states ${bad.join(", ")}, which your profile does not. Figures from the posting are the employer's and are never restated as yours. This is a bug; please report it.`);
        ...
        let note = "";
        try {
            await locked(() => addLetter({ id: randomBytes(4).toString("hex"), company: a.company, role: a.role, path, created: new Date().toISOString() }));
        }
        catch (e) {
            note = `\n\nThe file was written but could not be added to the letter history: ${String(e.message ?? e)}`;
        }
...
main().catch((e) => {
    process.stderr.write(`mcp-resume failed to start: ${String(e?.stack ?? e)}\n`);
    process.exit(1);
});
