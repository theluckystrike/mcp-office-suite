...
export function parseDates(s) {
    ...
    if (m)
        return { start: m[1].trim(), end: /present|current|now/i.test(m[2]) ? undefined : m[2].trim() };
...
export function parseRoleLine(line) {
...
export function blocksToProfile(blocks) {
    const profile = { name: "", email: "", experience: [], education: [] };
    const unparsed = [];
    const found = new Set();
    let section = "head";
    const skills = [];
    ...
    const pushItems = (items) => {
        if (section === "skills")
            skills.push(...items.flatMap(splitList));
        else if (section === "certifications")
            certs.push(...items);
        else if (section === "languages")
            langs.push(...items.flatMap(splitList));
        else if (section === "education")
            for (const it of items)
                education.push(eduOf(it));
        else if (section === "experience" && current)
    ...
    for (const b of blocks) {
        if (b.type === "heading") {
            ...
            if (section === "education") {
                education.push(eduOf(b.text));
                continue;
            }
            if (!profile.name) {
                profile.name = b.text.trim();
                continue;
            }
        ...
        if (b.type === "para") {
            ...
            if (section === "head") {
                if (!profile.name) {
                    profile.name = t.split("\n")[0].trim();
                    continue;
                }
                if (EMAIL.test(t) || PHONE.test(t) || /linkedin|github|https?:/i.test(t)) {
                    contact(t);
                    continue;
                }
                summary.push(t);
            ...
            if (section === "skills") {
                skills.push(...splitList(t));
                continue;
            }
            if (section === "languages") {
                langs.push(...splitList(t));
                continue;
            }
        ...
        if (b.type === "bullets") {
            pushItems(b.items.map((i) => i.trim()).filter(Boolean));
            continue;
        }
        if (b.type === "table") {
    ...
    if (!profile.email)
        contact(blocks.map((x) => (x.type === "para" ? x.text : "")).join("\n"));
    if (summary.length)
        profile.summary = summary.join(" ").trim();
    if (skills.length)
        profile.skills = dedupe(skills);
    if (certs.length)
        profile.certifications = dedupe(certs);
    if (langs.length)
        profile.languages = dedupe(langs);
...
function dedupe(a) {
    const seen = new Set();
    return a.filter((x) => { const k = x.toLowerCase(); if (seen.has(k))
