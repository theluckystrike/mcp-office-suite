 ...
 */
export function extractKeywords(jd, limit = 30, known) {
    const ws = words(jd);
    const allow = new Set(SHORT_SKILLS);
    for (const k of known ?? [])
        allow.add(k.toLowerCase());
    const uni = new Map();
    const bi = new Map();
    const usable = (w) => !!w && !STOP.has(w) && !/^\d+$/.test(w) && (w.length > 2 || allow.has(w));
     ...
     */
    const isSkill = (w) => allow.has(w) || allow.has(w.split(" ")[0]);
    const byCount = (a, b) => b[1] - a[1] || Number(isSkill(b[0])) - Number(isSkill(a[0])) ||
 ...
 */
export function suggestRewrites(p, keywords, limit = 5) {
    const out = [];
    for (const kw of keywords) {
        ...
        for (const e of p.experience) {
            ...
            if (i === 0)
                break; // already leads with it, nothing to do
            if (i <= 0)
                break;
...
export function analyseGap(p, jd, limit = 30) {
    ...
    for (const kw of keywords) {
        if (matchesKeyword(corpus, kw)) {
            matched.push(kw);
            if (matchesKeyword(skills, kw))
                inSkills.push(kw);
            if (matchesKeyword(bullets, kw))
                inBullets.push(kw);
        }
        else
            missing.push(kw);
