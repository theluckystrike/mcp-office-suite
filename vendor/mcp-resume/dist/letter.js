 ...
 */
export function prompt(what) { return `[add: ${what}]`; }
const OPENINGS = {
...
const SIGNOFFS = { formal: "Yours sincerely,", direct: "Regards,", warm: "Best wishes," };
 ...
 */
export function traceHighlight(h, p) {
    const corpus = norm(profileText(p));
    if (corpus.includes(norm(h)))
        return { ok: true, source: h };
    const ht = tokens(h);
    if (!ht.length)
        return { ok: false };
    const candidates = [...(p.skills ?? []), ...(p.certifications ?? [])];
    for (const e of p.experience)
        candidates.push(...e.bullets, `${e.title} ${e.company}`);
    let best = { score: 0, text: "" };
    for (const c of candidates) {
        const ct = new Set(tokens(c));
        const hit = ht.filter((t) => ct.has(t)).length / ht.length;
        if (hit > best.score)
            best = { score: hit, text: c };
    }
 ...
 */
export function unsourcedNumbers(letter, sources) {
 ...
 */
export function buildLetter(p, o) {
    const prompts = [];
    const use = (what) => { const s = prompt(what); prompts.push(s); return s; };
    ...
    if (p.summary)
        ...
        fit.push(`${use("one sentence on what you do")}`);
    ...
    const usedHighlights = [];
    const unverifiedHighlights = [];
    const proofLines = [];
    for (const h of o.highlights ?? []) {
        const t = traceHighlight(h, p);
        if (t.ok && t.source) {
            proofLines.push(t.source);
            usedHighlights.push(h);
        }
        else {
            unverifiedHighlights.push(h);
        }
    }
     ...
     */
    const roleOf = (line) => p.experience.findIndex((e) => e.bullets.includes(line));
    const groups = [];
    for (const line of proofLines) {
        ...
        if (last && last.exp === ei)
            ...
            groups.push({ exp: ei, lines: [line] });
    }
    const proofBlocks = [];
    const bulletItems = [];
    const withMetric = (line) => (numbersIn(line).length ? line : `${line} ${use("metric")}`);
    ...
    const blocks = [
        ...
        ...paras.map((text) => ({ type: "para", text })),
        ...proofBlocks,
        { type: "para", text: close },
        { type: "para", text: SIGNOFFS[o.tone] },
        { type: "para", text: p.name },
    ];
    const proofText = proofBlocks.map((b) => b.type === "bullets" ? b.items.map((i) => `- ${i}`).join("\n\n") : b.type === "para" ? b.text : "");
 ...
 */
export function rankBullets(p, keywords) {
    const scored = [];
    p.experience.forEach((e, ei) => {
        e.bullets.forEach((b, bi) => {
            let hits = 0;
            for (const k of keywords)
                if (matchesKeyword(b, k))
                    hits++;
