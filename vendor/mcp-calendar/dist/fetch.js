...
export function isBlockedHost(host) {
    ...
    if (h.endsWith(".local") || h.endsWith(".internal") || h.endsWith(".home.arpa"))
        return "a local network name";
    if (h === "metadata.google.internal")
        return "a cloud metadata service";
    if (h === "::1" || h === "0:0:0:0:0:0:0:1")
        return "the IPv6 loopback address";
    if (/^fe80:/i.test(h) || /^f[cd][0-9a-f]{2}:/i.test(h))
    ...
    if (v4) {
        ...
        if (a === 169 && b === 254)
            return "a link-local address (cloud metadata)";
        if (a === 172 && b >= 16 && b <= 31)
            return "a private address";
        if (a === 192 && b === 168)
            return "a private address";
        if (a === 100 && b >= 64 && b <= 127)
            return "a carrier-private address";
        if (a >= 224)
            return "a multicast or reserved address";
    }
    return null;
}
export async function fetchIcs(url, timeoutMs = TIMEOUT_MS) {
    let parsed;
    try {
        parsed = new URL(url.replace(/^webcal:/i, "https:"));
    }
    catch {
        throw new FetchError(`"${String(url).slice(0, 120)}" is not a valid URL. Include https://`);
    }
    ...
    if (blocked && !process.env.MCP_CALENDAR_ALLOW_LOCAL) {
        throw new FetchError(`refusing to fetch ${parsed.hostname}: that is ${blocked}. A calendar URL must be a public address. ` +
    ...
    clearTimeout(timer);
    if (!res.ok)
        throw new FetchError(`the feed returned HTTP ${res.status} for ${parsed.toString()}.`);
