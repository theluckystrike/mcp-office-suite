 ...
 */
export function decodeIcs(buf) {
    const out = Buffer.allocUnsafe(buf.length);
    let n = 0;
    for (let i = 0; i < buf.length; i++) {
        const b = buf[i];
        if (b === 0x0d && buf[i + 1] === 0x0a && (buf[i + 2] === 0x20 || buf[i + 2] === 0x09)) {
            i += 2;
            continue;
        }
        if ((b === 0x0d || b === 0x0a) && (buf[i + 1] === 0x20 || buf[i + 1] === 0x09)) {
            i += 1;
            continue;
        }
        out[n++] = b;
...
export function unescapeText(v) {
    ...
    for (let i = 0; i < v.length; i++) {
        ...
        if (n === "n" || n === "N")
            ...
            out += n; // unknown escape: keep the character itself
    }
    return out;
}
...
function dateValFrom(m, isDateParam, tzid) {
...
export function parseRRule(prop, warnings = []) {
    const parts = {};
    ...
    const rule = { freq, interval, wkst: DOW.indexOf((parts.WKST ?? "MO").toUpperCase()) < 0 ? 1 : DOW.indexOf((parts.WKST ?? "MO").toUpperCase()) };
    const count = Number(parts.COUNT);
    if (Number.isFinite(count) && count > 0)
        rule.count = Math.min(count, MAX_OCCURRENCES);
    if (parts.UNTIL) {
...
export function parseIcs(text, opts = {}) {
    if (typeof text !== "string" || !text.trim())
        throw new IcsError("the calendar file is empty.");
    const lines = unfold(text);
    const warnings = [];
    const seenWarn = new Set();
    const warn = (w) => { if (!seenWarn.has(w)) {
        seenWarn.add(w);
        warnings.push(w);
    } };
    ...
    for (const line of lines) {
        ...
        if (p.name === "BEGIN") {
            ...
            if (cur || ignoreDepth > 0) {
                ignoreDepth++;
                continue;
            } // VALARM inside a VEVENT
            if (comp === "VEVENT") {
                cur = {};
                continue;
            }
            ...
            if (comp === "VEVENT" && cur) {
                if (events.length < maxEvents) {
                    try {
                        ...
                        skipped++;
                        warn(`skipped an event: ${e.message}`);
                    }
                }
                else {
                    skipped++;
        ...
        if (p.name === "X-WR-CALNAME")
            calendarName = unescapeText(p.value).trim() || undefined;
        if (p.name === "PRODID")
            prodId = unescapeText(p.value).trim() || undefined;
    }
    ...
    if (skipped)
        warn(`${skipped} event(s) could not be read and were skipped.`);
    applyOverrides(events);
    return { events, calendarName, prodId, warnings, skipped };
}
function buildEvent(props, warn) {
    const one = (n) => props[n]?.[0];
    const dtstartP = one("DTSTART");
    if (!dtstartP)
        throw new Error("no DTSTART");
    const localWarn = [];
    const start = parseDateValue(dtstartP, localWarn);
    if (!start)
        throw new Error(`DTSTART "${dtstartP.value.slice(0, 40)}" is not a date`);
    for (const w of localWarn)
        warn(w);
    let end = null;
    let durationMs = 0;
    ...
    if (dtendP) {
        ...
        if (!end)
            warn(`DTEND "${dtendP.value.slice(0, 40)}" is not a date; that event is treated as ${start.allDay ? "a whole day" : "zero length"}.`);
    }
    if (!end && durP) {
        ...
        if (ms !== null && ms >= 0)
            ...
            warn(`DURATION "${durP.value.slice(0, 40)}" could not be read.`);
    }
    if (!end && !durationMs) {
    ...
    for (const rd of props.RDATE ?? []) {
        if ((rd.params.VALUE ?? []).some(v => v.toUpperCase() === "PERIOD"))
            continue; // periods are not expanded
...
function applyOverrides(events) {
    ...
    for (const e of events) {
        ...
        if (master)
            master.exdates.push(e.recurrenceId);
    }
}
const P2 = (n) => String(n).padStart(2, "0");
export function wallKey(w, allDay) {
 ...
 */
export function instantOf(w, zone) {
    try {
        return zonedToUtc(w, zone, { gap: "forward", fold: "first" });
    }
    catch (e) {
        if (e instanceof UnknownZoneError)
            return zonedToUtc(w, "UTC", { gap: "forward", fold: "first" });
        throw e;
    }
}
export function zoneOfEvent(e, defaultZone) {
...
function applySetPos(list, pos) {
    if (!pos?.length)
        return list;
    const out = [];
    for (const p of pos) {
        const i = p > 0 ? p - 1 : list.length + p;
        if (i >= 0 && i < list.length && !out.includes(list[i]))
            out.push(list[i]);
    }
    return out;
}
/** Every start wall clock the rule produces, in order, starting at DTSTART. */
function* ruleWalls(start, r) {
    let steps = 0;
    const guard = () => { if (++steps > MAX_CANDIDATES)
        throw new IcsError("this recurrence rule produces too many dates to expand; narrow the window."); };
    if (r.freq === "HOURLY") {
        for (let n = 0;; n++) {
            guard();
            yield addHoursWall(start, n * r.interval);
        }
    }
    if (r.freq === "DAILY") {
        for (let n = 0;; n++) {
            guard();
            yield addDaysWall(start, n * r.interval);
        }
    }
    if (r.freq === "WEEKLY") {
    ...
    if (r.freq === "MONTHLY") {
        for (let n = 0;; n++) {
            ...
            for (const d of applySetPos(days, r.bySetPos)) {
                guard();
                const cand = { y, m, d, h: start.h, mi: start.mi, s: start.s };
                if (wallLE(start, cand))
                    yield cand;
            }
            guard();
    ...
    for (let n = 0;; n++) {
        ...
        for (const m of months) {
            ...
            for (const d of days)
                yearCands.push({ y, m, d, h: start.h, mi: start.mi, s: start.s });
        }
...
export function expandEvent(e, o) {
    ...
    const emit = (w) => {
        ...
        if (excluded.has(key) || excluded.has(wallKey(w, true)))
            return true;
        const startUtc = instantOf(w, zone);
        const endUtc = new Date(startUtc.getTime() + span);
        if (endUtc.getTime() <= o.fromUtc.getTime())
            return true;
        if (startUtc.getTime() >= o.toUtc.getTime())
            return true;
    ...
    if (!e.rrule) {
        ...
        for (const w of ruleWalls(e.start.wall, r)) {
            ...
            if (untilMs !== undefined && startUtc.getTime() > untilMs)
                break;
            n++;
            if (r.count !== undefined && n > r.count)
                break;
    ...
    for (const rd of e.rdates) {
        ...
        const endUtc = new Date(startUtc.getTime() + span);
        if (endUtc <= o.fromUtc || startUtc >= o.toUtc)
            continue;
        const key = wallKey(rd.wall, rd.allDay);
        if (excluded.has(key))
            continue;
        if (out.some(x => x.startUtc.getTime() === startUtc.getTime()))
            continue;
        out.push({ event: e, startUtc, endUtc, allDay, key, zone });
    }
...
export function eventSpanMs(e, zone) {
    if (e.durationMs)
        return e.durationMs;
...
export function expandAll(events, o) {
    const out = [];
    const limit = Math.min(o.limit ?? MAX_OCCURRENCES, MAX_OCCURRENCES);
    for (const e of events) {
 ...
 */
export function occurrenceId(calSlug, occ) {
...
export function occurrenceFromKey(e, key, defaultZone) {
...
export function mergeBlocks(occ) {
    const sorted = [...occ].sort((a, b) => a.startUtc.getTime() - b.startUtc.getTime());
    const out = [];
    for (const o of sorted) {
        const last = out[out.length - 1];
        if (last && o.startUtc.getTime() <= last.endUtc.getTime()) {
            if (o.endUtc > last.endUtc)
                last.endUtc = o.endUtc;
            if (!last.labels.includes(o.label))
                last.labels.push(o.label);
        }
        else {
