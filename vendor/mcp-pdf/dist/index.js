...
import { z } from "zod";
import { MAX_BYTES, expandPath, humanBytes, loadPdf, looksEncrypted, parsePageList, parseRanges, releaseReservations, reserveOutput, } from "./pdfio.js";
 ...
 */
async function record(op, inputs, outputs, pages) {
    try {
        await locked(() => addOp({
        ...
        return `\n\nThe file was written, but it could not be added to the operation history: ${String(e.message ?? e)}`;
    }
}
const COLORS = {
 ...
 */
export function sanitizeStampText(s) {
    const map = {
        "‘": "'", "’": "'", "“": '"', "”": '"',
        "–": "-", "—": "-", "…": "...", " ": " ",
    };
...
function place(position, w, h, textWidth, textHeight) {
...
function mm(pt) { return Math.round((pt * 25.4) / 72 * 10) / 10; }
function paperName(w, h) {
    ...
    for (const [name, pw, ph] of pairs) {
        if ((near(w, pw) && near(h, ph)))
            return name;
        if ((near(w, ph) && near(h, pw)))
            return `${name} landscape`;
    }
    return "custom";
}
async function savePdf(doc, path) {
    const bytes = await doc.save({ useObjectStreams: false });
    writeFileSync(path, bytes);
    return bytes.length;
}
/** Free tier refusals are answers, not protocol errors: the model has to relay the upgrade line. */
function freeLimit(text) { return ok(text); }
const server = new McpServer({ name: "mcp-pdf", version: VERSION }, { capabilities: { tools: {}, resources: {}, prompts: {} } });
/* --------------------------------------------------------------------- info */
...
server.registerTool("pdf_count", {
    ...
    for (const p of paths) {
        try {
            ...
            rows.push({ file: expandPath(p), pages: null, error: String(e.message ?? e) });
        }
    }
...
server.registerTool("pdf_merge", {
    ...
    try {
        if (!gate.isPro() && paths.length > FREE_MAX_MERGE_FILES) {
            return freeLimit(`You asked to merge ${paths.length} files. The free tier merges up to ${FREE_MAX_MERGE_FILES} files in one call ` +
        ...
        return ok(`Merged ${loaded.length} files into ${out.getPageCount()} pages.\n\n` +
...
server.registerTool("pdf_split", {
    ...
    try {
        ...
        if (!/\{n\}|\{range\}/.test(out_path_pattern) && parts.length > 1) {
            throw new Error(`out_path_pattern "${out_path_pattern}" has no {n} or {range} placeholder, so all ${parts.length} parts would ` +
        ...
        for (const t of targets) {
            ...
            if (seen.has(full))
                throw new Error(`the pattern produces the path ${full} twice; nothing was written.`);
            seen.add(full);
        }
        ...
        for (const t of targets)
            reserved.push(reserveOutput(t, overwrite ?? false, [path]));
        const written = [];
        ...
        return ok(`Split ${f.path} (${f.pageCount} pages) into ${written.length} file${written.length === 1 ? "" : "s"}.\n\n` +
...
function freePageText(tool, pageCount) {
...
server.registerTool("pdf_pages", {
    ...
    try {
        const f = await loadPdf(path);
        if (!gate.isPro() && f.pageCount > FREE_MAX_PAGES)
            return freeLimit(freePageText("pdf_pages", f.pageCount));
        ...
        return ok(`Extracted ${idx.length} page${idx.length === 1 ? "" : "s"} from ${f.path}.\n\n` +
...
server.registerTool("pdf_rotate", {
    ...
    try {
        ...
        if (!gate.isPro() && f.pageCount > FREE_MAX_PAGES)
            return freeLimit(freePageText("pdf_rotate", f.pageCount));
        ...
        return ok(`Rotated ${changed.length} page${changed.length === 1 ? "" : "s"} by ${a.degrees} degrees.\n\n` +
...
async function stamp(a, businessFooter) {
    let reservation = null;
    try {
        const preset = STAMP_PRESETS[a.text.trim().toUpperCase()];
        if (!gate.isPro()) {
            if (!preset) {
                return freeLimit(`The free tier stamps the two presets PAID and DRAFT, and nothing was written. You asked for "${a.text}". ` +
                    `Stamp PAID or DRAFT, or unlock any stamp text.\n\n${gate.upgradeText("custom stamp text")}`);
            }
            if (a.color) {
                return freeLimit(`A custom stamp colour is a Pro feature and nothing was written. The PAID preset is ${STAMP_PRESETS.PAID.note} ` +
        ...
        if (a.font_size !== undefined && (!Number.isFinite(a.font_size) || a.font_size <= 0 || a.font_size > MAX_FONT_SIZE)) {
            throw new Error(`font_size must be greater than 0 and at most ${MAX_FONT_SIZE} points; got ${a.font_size}. ` +
        ...
        if (!clean.text)
            throw new Error(`the stamp text is empty after removing characters a built-in PDF font cannot carry. Nothing was written.`);
        const f = await loadPdf(a.path);
        const idx = a.pages ? parsePageList(a.pages, f.pageCount) : f.doc.getPageIndices();
        const colour = parseColor(a.color ?? preset?.color ?? "#b02020");
        const opacity = a.opacity ?? (a.position === "center" || !a.position ? 0.35 : 0.85);
        if (opacity <= 0 || opacity > 1)
            throw new Error(`opacity must be greater than 0 and at most 1; got ${opacity}.`);
        reservation = reserveOutput(a.out_path, a.overwrite ?? false, [a.path]);
        ...
        for (const i of new Set(idx)) {
            ...
            if (tw > budget)
                overflow.push({ page: i + 1, text_pt: Math.round(tw), page_pt: Math.round(budget), size });
        ...
        return ok(`Stamped "${clean.text}" on ${stamped.length} page${stamped.length === 1 ? "" : "s"}.\n\n` +
            JSON.stringify({
...
function autoSize(text, pageWidth, position, font) {
    const budget = position === "center" ? pageWidth * 0.95 : pageWidth - 96;
    let size = position === "center" ? 72 : 24;
    while (size > 6 && font.widthOfTextAtSize(text, size) > budget)
        size -= 2;
    return size;
}
...
server.registerTool("pdf_watermark_business", {
    ...
    try {
        if (!gate.isPro()) {
            return freeLimit(`The business footer is a Pro feature and nothing was written. On the free tier, pdf_stamp with the PAID or ` +
        ...
        if (!name) {
            return fail(`no business name is stored, so there is nothing to put in the footer and nothing was written. ` +
...
server.registerTool("pdf_reorder", {
    ...
    try {
        if (!gate.isPro()) {
            return freeLimit(`Reordering pages is a Pro feature and nothing was written. On the free tier, pdf_pages extracts pages in any ` +
        ...
        for (const n of a.order) {
            if (n > f.pageCount)
                throw new Error(`page ${n} does not exist: the file has ${f.pageCount} pages. Nothing was written.`);
        ...
        if (dupes.length || missing.length) {
            throw new Error(`the order must name each of the ${f.pageCount} pages exactly once and nothing was written. ` +
        ...
        return ok(`Reordered ${f.pageCount} pages.\n\n` +
...
server.registerTool("pdf_text", {
    ...
    try {
        ...
        for (const b of blocks) {
            if (used + b.length > MAX_TEXT_CHARS && shown > 0)
                break;
            used += b.length + 2;
            shown++;
        }
        ...
        const header = `${f.path}: text from ${withText} of ${idx.length} page${idx.length === 1 ? "" : "s"} ` +
    ...
    for (const fl of fields) {
        ...
        try {
            if (typeof anyF.getText === "function")
                ...
                value = anyF.isChecked() ? "checked" : "unchecked";
            else if (typeof anyF.getSelected === "function")
                value = (anyF.getSelected() ?? []).join(", ");
        }
        catch {
...
server.registerResource("recent-operations", "pdf://recent", {
    ...
    let ops;
    try {
        ops = getOps().slice(-25).reverse();
    }
    catch (e) {
        ops = { error: String(e.message ?? e) };
    }
...
server.registerPrompt("mark_invoice_paid", {
    ...
    const text = `Mark invoice ${reference} as paid.\n\n` +
        (path
