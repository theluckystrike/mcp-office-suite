...
export async function loadPdf(input) {
    ...
    if (!existsSync(path))
        throw new Error(`${path} does not exist. Give the full path to an existing PDF file.`);
    const st = statSync(path);
    if (st.isDirectory())
        throw new Error(`${path} is a directory, not a PDF file.`);
    if (st.size === 0)
        throw new Error(`${path} is empty (0 bytes); nothing to read.`);
    if (st.size > MAX_BYTES) {
        throw new Error(`${path} is ${humanBytes(st.size)}; this server refuses inputs over ${humanBytes(MAX_BYTES)} ` +
    ...
    try {
        ...
        if (/encrypt/i.test(msg) || /password/i.test(msg)) {
            throw new Error(`${path} is encrypted (password or permissions protected) and was refused. ` +
 ...
 */
export function reserveOutput(out, overwrite, inputs = [], ext = ".pdf") {
    ...
    for (const raw of inputs) {
        ...
        if (inPath === withExt || sameFile(inPath, withExt)) {
            throw new Error(`out_path ${withExt} is also an input of this operation, so writing it would destroy the source ` +
    ...
    if (overwrite)
        return { path: withExt, created: false };
    try {
        ...
        if (e.code !== "EEXIST")
            throw e;
        throw new Error(`${withExt} already exists and nothing was written. ` +
            `Pass overwrite: true to replace it, or give a different out_path.`);
    }
}
export function releaseReservations(rs) {
    for (const r of rs) {
        if (!r.created)
            continue;
        try {
            if (statSync(r.path).size === 0)
                unlinkSync(r.path);
        }
        catch { /* leave it */ }
    }
 ...
 */
export function pdfaClaim(bytes) {
    const s = Buffer.from(bytes).toString("latin1");
...
export function parseRanges(spec, pageCount) {
    ...
    if (!parts.length)
        throw new Error(`empty page range. Use something like "1-3,5,7-".`);
    for (const part of parts) {
        const m = /^(\d+)?\s*(-)?\s*(\d+)?$/.exec(part);
        if (!m || (!m[1] && !m[3]))
            throw new Error(`cannot read "${part}" as a page range. Use "1-3", "5", "7-" or "-4".`);
        const open = !!m[2];
        ...
        if (from < 1 || to < 1)
            throw new Error(`page numbers start at 1; "${part}" does not.`);
        ...
        if (from > to)
            throw new Error(`"${part}" runs backwards; write it as ${to}-${from}.`);
