...
import { PDFArray, PDFName, PDFRawStream, PDFStream } from "pdf-lib";
function decodeStream(stream) {
    if (!(stream instanceof PDFRawStream))
        return { bytes: null, filter: "not a raw stream" };
    const raw = Buffer.from(stream.contents);
    const f = stream.dict.lookup(PDFName.of("Filter"));
    const names = [];
    if (f instanceof PDFName)
        names.push(f.asString());
    else if (f instanceof PDFArray) {
        for (let i = 0; i < f.size(); i++) {
            const n = f.lookup(i);
            if (n instanceof PDFName)
                names.push(n.asString());
        }
    }
    if (!names.length)
        return { bytes: raw, filter: "none" };
    if (names.length === 1 && (names[0] === "/FlateDecode" || names[0] === "/Fl")) {
        try {
            return { bytes: inflateSync(raw), filter: "FlateDecode" };
        }
        catch { /* try raw deflate */ }
        try {
            return { bytes: inflateRawSync(raw), filter: "FlateDecode (headerless)" };
        }
        catch { /* corrupt */ }
...
function pageStreams(doc, pageIndex) {
    ...
    if (contents instanceof PDFArray) {
        ...
        const s = doc.context.lookupMaybe(contents, PDFStream);
        if (s)
            out.push(s);
    }
    return out;
...
export function textFromContentStream(content) {
    ...
    let out = "";
    let stack = [];
    const push = (o) => { stack.push(o); if (stack.length > 4096)
        stack = stack.slice(-64); };
    const emit = (t) => { if (t)
        out += t; };
    ...
    while (i < s.length) {
            ...
            while (i < s.length) {
                ...
                if (ch === "\\") {
                    ...
                    if (n === "n")
                        ...
                        let oct = n;
                        while (oct.length < 3 && s[i] >= "0" && s[i] <= "7") {
                            oct += s[i];
                            i++;
                        }
        ...
        if (c === "/") {
            i++;
            while (i < s.length && !isSpace(s[i]) && !DELIM.has(s[i]))
                i++;
            push({ kind: "other" });
            continue;
        }
        ...
        if (/^[+-]?(\d+\.?\d*|\.\d+)$/.test(tok)) {
            push({ kind: "number", value: Number(tok) });
            continue;
        }
...
export function extractText(doc, pageIndexes) {
    const pages = [];
    const warnings = [];
    for (const idx of pageIndexes) {
        ...
        for (const st of streams) {
            ...
            if (d.bytes)
                buf = Buffer.concat([buf, d.bytes, Buffer.from("\n")]);
            else
                skipped.push(d.filter);
        }
        if (skipped.length) {
        ...
        try {
            ...
            warnings.push(`page ${idx + 1}: content stream could not be walked (${String(e.message ?? e)})`);
        }
        text = text.replace(/[ \t]+\n/g, "\n").replace(/\n{3,}/g, "\n\n").trim();
        const note = text
            ? undefined
    ...
    if (garbled.length) {
        warnings.push(`page${garbled.length > 1 ? "s" : ""} ${garbled.map((p) => p.page).join(", ")}: the operators produced no readable ` +
