...
export function perEur(rates, code) {
 ...
 * as the multiplier.
 */
export function convertAmount(rates, amount, from, to) {
    const f = from.toUpperCase(), t = to.toUpperCase();
    const fr = perEur(rates, f), tr = perEur(rates, t);
    if (fr === undefined)
        return { error: unknownCode(f, rates) };
    if (tr === undefined)
        return { error: unknownCode(t, rates) };
...
export function unknownCode(code, rates) {
    return `"${code}" is not in the ECB reference set. The ECB quotes ${codesOf(rates).length} currencies: ${codesOf(rates).join(", ")}.`;
}
export function resolveDate(days, asked) {
    if (Object.prototype.hasOwnProperty.call(days, asked))
        return { date: asked, rates: days[asked], exact: true, asked };
    const dates = Object.keys(days).sort();
    if (!dates.length)
        return { error: "the rate history cache is empty." };
...
export function series(days, from, to, fromDate, toDate) {
    const out = [];
    const f = from.toUpperCase(), t = to.toUpperCase();
    for (const d of Object.keys(days).sort()) {
        ...
        if (fr === undefined || tr === undefined)
            continue; // currency not quoted that day (a pre-euro legacy code)
        out.push({ date: d, rate: crossRate(fr, tr) });
    }
    if (!out.length)
        return { error: `no ECB rate for ${f}/${t} between ${fromDate} and ${toDate}.` };
    return out;
}
