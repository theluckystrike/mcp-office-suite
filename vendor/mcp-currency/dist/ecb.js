...
import { dailyPath, historyPath, loadDaily, loadHistory, lockPath, writeJsonFile, } from "./store.js";
...
export const PUBLISH_NOTE = "ECB reference rates are published around 16:00 CET on TARGET business days; there is no rate for a weekend or a TARGET holiday.";
/**
 ...
 */
export function assertComplete(xml) {
...
export function parseEcbXml(xml) {
    ...
    TIME_RE.lastIndex = 0;
    let m;
    while ((m = TIME_RE.exec(xml)))
        marks.push({ date: m[1], at: m.index });
    if (!marks.length)
        throw new EcbError("the ECB file held no dated rate block; the download was truncated or is not the eurofxref file.");
    const out = [];
...
/* ---------------------------------------------------------------------- fetch */
export async function fetchText(url, totalTimeoutMs = TIMEOUT_MS) {
    ...
    const fail = (e) => {
        ...
        return new EcbError(`could not reach the ECB (${e?.message ?? "network error"})`);
    };
    try {
        let res;
        try {
        ...
        if (!res.ok)
            throw new EcbError(`the ECB returned HTTP ${res.status} for ${url}`);
        const body = res.body;
        if (!body) {
            try {
 ...
 */
async function readThrough(label, maxAgeMs, read, file, build) {
    const cached = read();
    if (cached && ageMs(cached.fetched_at) < maxAgeMs)
        return { data: cached, refreshed: false };
    try {
        ...
        return await withFileLock(lockPath(), async () => {
            const again = read();
            if (again && ageMs(again.fetched_at) < maxAgeMs)
                return { data: again, refreshed: false };
        ...
        if (cached) {
            return {
                data: cached,
                refreshed: false,
                offline_note: `could not refresh the ${label} from the ECB (${e.message}); using the copy cached at ${cached.fetched_at}.`,
            };
        }
        throw e;
    }
...
export function getDaily() {
    return readThrough("daily rates", DAILY_MAX_AGE_MS, loadDaily, dailyPath(), async () => {
        const xml = await fetchText(DAILY_URL(), TIMEOUT_MS);
        assertComplete(xml);
        const days = parseEcbXml(xml);
        const latest = days.reduce((a, b) => (a.date >= b.date ? a : b));
        if (!Object.keys(latest.rates).length)
            throw new EcbError("the ECB daily file held a date but no rates.");
...
export function getHistory() {
    return readThrough("rate history", HISTORY_MAX_AGE_MS, loadHistory, historyPath(), async () => {
        const xml = await fetchText(HISTORY_URL(), HISTORY_TIMEOUT_MS);
        assertComplete(xml);
        const days = parseEcbXml(xml);
        const map = {};
        for (const d of days)
            if (Object.keys(d.rates).length)
                map[d.date] = d.rates;
        if (!Object.keys(map).length)
            throw new EcbError("the ECB history file held no rates.");
