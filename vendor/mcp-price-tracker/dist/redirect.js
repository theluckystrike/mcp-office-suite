...
export function pathSegments(u) {
    try {
 ...
 */
export function productToken(u) {
 ...
 */
export function keepsProductIdentity(requestedUrl, finalUrl) {
    ...
    if (want.length > 0 && want.every((s) => gotSet.has(s.toLowerCase())))
        return true;
    const token = productToken(requestedUrl);
    if (token && finalUrl.toLowerCase().includes(token.toLowerCase()))
        return true;
    return false;
}
/**
 ...
 */
export function checkRedirect(requestedUrl, finalUrl, title) {
    if (!finalUrl || sameUrl(requestedUrl, finalUrl))
        return { ok: true };
    const reason = (why) => ({
        ok: false,
        reason: `the shop redirected to ${finalUrl}, which is not a product page (${why}). ` +
    ...
    if (got.length === 0 && want.length > 0)
        return reason("it is the shop home page");
    ...
    const listing = got.find((s) => {
        ...
        if (!CATEGORY_SEGMENTS.has(seg) || wantSet.has(seg))
            return false;
        // A product route plus a surviving product identity plus a real product
        // title is a canonical product URL, not a listing.
        if (PRODUCT_ROUTE_SEGMENTS.has(seg) && sameProduct && namedProduct)
            return false;
        return true;
    });
    if (listing)
        return reason(`it is a "${listing}" listing page`);
    if (t) {
        if (NOT_FOUND.test(t))
            return reason(`the page title is "${t}"`);
        // "Name - Shop" was already reduced to its head above.
        if (GENERIC_TITLE.test(head) || GENERIC_TITLE.test(t))
            return reason(`the page title is "${t}"`);
