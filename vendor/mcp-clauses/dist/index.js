...
import { buildDocx, letterhead, stripInvalidXml } from "@theluckystrike/mcp-docx/lib";
import { z } from "zod";
import { CATEGORY_ORDER, categoryRank, dataDir, load, lockPath, save, STARTER_NOTE, } from "./store.js";
import { DISCLAIMER, clauseVariables, makeId, orderByCategory, parseClauseJson, parseMarkdown, promptFor, search, toClauseJson, toMarkdown, variablesFor, } from "./library.js";
 ...
 */
function findClause(clauses, ref) {
    ...
    if (partial.length > 1) {
        throw new Error(`"${ref}" matches more than one clause: ${partial.map((c) => `${c.id} (${c.title})`).join(", ")}. ` +
...
function letterheadNote() {
    const p = readSharedProfile();
    if (!p.name)
        return " No business profile is stored, so the letterhead is blank: run business_set (invoice or docx) once and assemble again.";
...
server.registerTool("clause_get", {
    ...
    try {
        if (!a.id && !a.title)
            return fail("pass id or title");
        const c = findClause(load().clauses, (a.id ?? a.title));
...
server.registerTool("clause_update", {
    ...
    try {
        return await locked(() => {
            ...
            if (!c)
                return fail(`no clause matches "${a.id}"`);
            const pro = gate.isPro();
            const changedText = (a.body !== undefined && a.body !== c.body) || (a.title !== undefined && a.title !== c.title);
            if (pro && changedText)
                c.history.push({ at: c.updated, title: c.title, body: c.body });
            if (a.title !== undefined)
                c.title = a.title.trim();
            if (a.body !== undefined)
                c.body = a.body;
            if (a.category !== undefined)
                c.category = slugCategory(a.category);
            if (a.tags !== undefined)
                c.tags = a.tags;
            if (a.variables !== undefined)
                c.variables = a.variables;
            if (a.jurisdiction !== undefined)
                c.jurisdiction = a.jurisdiction;
...
server.registerTool("clause_import", {
    ...
    try {
        ...
        if (!parsed.length)
            return fail(`no clauses found in ${file}`);
        return await locked(() => {
            const db = load();
            const pro = gate.isPro();
            ...
            for (const p of parsed) {
                ...
                if (existing) {
                    ...
                    if (pro && existing.body !== p.body)
                        existing.history.push({ at: existing.updated, title: existing.title, body: existing.body });
                    existing.body = p.body;
                    existing.category = slugCategory(p.category);
                    existing.tags = p.tags;
                    existing.variables = p.variables;
                    existing.jurisdiction = p.jurisdiction;
                    existing.language = p.language;
                    existing.updated = now;
                    replaced++;
                    continue;
                }
                if (!pro && db.clauses.filter((c) => !c.starter).length >= FREE_OWN_CLAUSES) {
                    capped++;
                    continue;
                }
                db.clauses.push({
...
/* ----------------------------------------------------------------- assembly */
function selectClauses(all, ids, categories) {
    if (ids?.length) {
        const picked = [];
        for (const ref of ids) {
            const c = findClause(all, ref);
            if (!c)
                return { error: `no clause matches "${ref}"` };
    ...
    if (categories?.length) {
        ...
        if (!picked.length)
            return { error: `no clauses in ${want.join(", ")}` };
        return { picked };
    }
    return { error: "pass clause_ids or categories" };
}
function assembleNote(pro, unfilled, missing = []) {
    const parts = [`The document opens with the not-legal-advice line: ${DISCLAIMER}`];
    if (unfilled)
        parts.push(`${unfilled} fact(s) had no value and are left in the document as bracketed prompts; fill them before sending.`);
    if (missing.length) {
        parts.push(`${missing.length} cross-reference(s) were DROPPED because the clause they point at is not in this document ` +
...
server.registerTool("contract_assemble", {
    ...
    try {
        ...
        if (error || !picked)
            return fail(error ?? "no clauses selected");
        const pro = gate.isPro();
