...
export const DISCLAIMER = "Generic template, not legal advice. Have a qualified lawyer review this document before you sign it.";
...
export function clauseVariables(c) {
    const out = [...c.variables];
    for (const v of extractVariables(c.body))
        if (!out.includes(v))
            out.push(v);
    return out;
}
/** The union of the variables of several clauses, ordered by first appearance. */
export function variablesFor(clauses) {
    const out = [];
...
export function fillVariables(text, values) {
    const filled = [];
    const unfilled = [];
    const out = text.replace(VAR_RE, (_m, name) => {
        const v = values[name];
...
function countWordMatches(text, term) {
 ...
 */
export function search(clauses, query, o = {}) {
    const terms = query.toLowerCase().split(/\s+/).filter(Boolean);
    const pool = clauses.filter((c) => {
        if (o.category && c.category !== o.category)
            return false;
        if (o.jurisdiction && (c.jurisdiction ?? "").toLowerCase() !== o.jurisdiction.toLowerCase())
            return false;
        if (o.tags && o.tags.length) {
            const have = c.tags.map((t) => t.toLowerCase());
            if (!o.tags.every((t) => have.includes(t.toLowerCase())))
                return false;
        }
        return true;
    });
    ...
    for (const clause of pool) {
        ...
        for (const t of terms) {
            if (title === t)
                score += 100;
            else if (title.split(/\W+/).includes(t))
                score += 60;
            else if (countWordMatches(title, t))
                score += 60;
            else if (title.includes(t))
                score += 5; // substring fallback, ranked well below a real match
            if (tags.includes(t))
                score += 30;
            else if (tags.some((x) => x.includes(t)))
                score += 15;
            if (clause.category.toLowerCase() === t)
                score += 20;
            if (clause.id.includes(t))
                score += 10;
            const wordHits = countWordMatches(body, t);
            if (wordHits)
                score += Math.min(wordHits, 5) * 4;
            else {
                const substringHits = body.split(t).length - 1;
                if (substringHits)
                    score += Math.min(substringHits, 5) * 1; // substring fallback
            }
        }
        if (score > 0)
            hits.push({ clause, score });
    }
...
export function orderByCategory(clauses) {
 ...
 */
function looksLikeMetadataValue(v) {
    const t = v.trim();
    if (!t)
        return true;
...
export function parseMarkdown(text) {
    ...
    for (const part of parts) {
        ...
        while (lines.length) {
            ...
            if (!m || !HEADER_KEYS.has(m[1]) || !looksLikeMetadataValue(m[2]))
                break;
            meta[m[1]] = m[2].trim();
            lines.shift();
        }
        ...
        if (!body)
            continue;
        const list = (k) => (meta[k] ? meta[k].split(",").map((s) => s.trim()).filter(Boolean) : []);
        out.push({
            title, body,
...
export function toMarkdown(clauses, heading = "Clause library") {
    ...
    for (const c of clauses) {
        ...
        if (c.variables.length)
            lines.push(`variables: ${c.variables.join(", ")}`);
        if (c.jurisdiction)
...
export function parseClauseJson(text) {
    const raw = JSON.parse(text);
    const arr = Array.isArray(raw) ? raw : Array.isArray(raw.clauses) ? raw.clauses : undefined;
    if (!arr)
        throw new Error("expected a JSON array of clauses, or an object with a clauses array");
    return arr.map((r) => {
        const c = r;
        if (typeof c.title !== "string" || typeof c.body !== "string")
            throw new Error("every clause needs a title and a body");
        return {
...
export function toClauseJson(clauses) {
