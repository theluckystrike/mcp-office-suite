 ...
 */
export function parseNumberBody(input) {
    const s = input.replace(SPACES, " ").trim();
    if (s === "" || !/\d/.test(s))
        return null;
    if (EU_DECIMAL.test(s))
        return finite(Number(s.replace(/[. ]/g, "").replace(",", ".")));
    if (EN_GROUPED.test(s))
        return finite(Number(s.replace(/,/g, "")));
    if (SPACE_GROUPED.test(s))
        return finite(Number(s.replace(/ /g, "")));
    if (PLAIN.test(s))
 ...
 */
export function parseNumberStrict(raw) {
 ...
 */
export function parseNumberLoose(v, opts = {}) {
    if (typeof v === "number")
    ...
    if (opts.identifiers && looksLikeIdentifier(s))
        return null;
    const n = parseNumberBody(s);
    if (n === null)
        return null;
 ...
 */
export function parseNumberForCompare(v) {
