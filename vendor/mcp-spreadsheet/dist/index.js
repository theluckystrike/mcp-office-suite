...
function guard(fn) {
    return async (...a) => {
        try {
            ...
            const msg = e instanceof UserError || e instanceof ExprError ? e.message : `${e.message}`;
            return fail(msg);
        }
    };
}
...
function formulaColumns(formula, headers) {
    let used;
    try {
        used = columnsUsed(parse(formula));
    }
    catch {
        return [];
    }
 ...
 */
function maxDecimals(recs, cols) {
    if (!cols.length)
        return null;
    let seen = false;
    let max = 0;
 ...
 */
function rowBudget(q) {
    if (q.where || q.sort || q.range)
        return undefined;
    if ((q.group_by && q.group_by.length) || (q.aggregate && q.aggregate.length))
        return undefined;
    return (q.offset ?? 0) + (q.limit ?? 100);
}
function open(path, sheet, budget) {
 ...
 */
function writeMatrix(file, headers, rows, sheetName = "Sheet1", base) {
    const ext = extname(file).toLowerCase();
    const aoa = [headers, ...rows];
    if (ext === ".csv" || ext === ".txt")
        writeAtomic(file, toCsv(aoa, ","));
    else if (ext === ".tsv")
        writeAtomic(file, toCsv(aoa, "\t"));
    else if (ext === ".json")
        writeAtomic(file, JSON.stringify(rows.map((r) => Object.fromEntries(headers.map((h, i) => [h, jsonCell(r[i] ?? null)]))), null, 2));
    else {
        const ws = XLSX.utils.aoa_to_sheet(aoa, { cellDates: true });
        const name = sheetName.slice(0, 31) || "Sheet1";
        if (base && base.SheetNames.length) {
            const wb = { ...base, SheetNames: base.SheetNames.slice(), Sheets: { ...base.Sheets } };
            wb.Sheets[name] = ws;
            if (!wb.SheetNames.includes(name))
                wb.SheetNames.push(name);
            writeAtomic(file, XLSX.write(wb, { type: "buffer", bookType: "xlsx", cellDates: true }));
            return;
        }
...
function normaliseRows(rows, existingHeaders) {
    if (!Array.isArray(rows) || rows.length === 0)
        throw new UserError("rows must be a non-empty array");
    const first = rows[0];
    if (Array.isArray(first)) {
        const matrix = rows.map((r) => r.map((c) => (c === undefined ? null : c)));
        if (existingHeaders) {
...
const server = new McpServer({ name: "mcp-spreadsheet", version: VERSION }, { capabilities: { tools: {}, resources: {}, prompts: {} } });
gate.registerTools(server);
// ---------------------------------------------------------------- sheet_info
async function infoText(path) {
    const pro = gate.isPro();
    const wb = loadWorkbook(path, { maxRows: pro ? Infinity : FREE_MAX_ROWS });
    const out = { file: wb.path, format: wb.kind, sizeBytes: wb.bytes, sheets: [] };
    if (wb.delimiter)
        out.delimiter = wb.delimiter === "\t" ? "tab" : wb.delimiter;
...
}, guard(async ({ path }) => text(await infoText(path))));
// ---------------------------------------------------------------- sheet_read
server.registerTool("sheet_read", {
...
}, guard(async ({ path, sheet, range, limit, offset, as }) => {
    ...
    if (fmt === "json")
        return withNotes(o.notes, JSON.stringify(rows.map((r) => Object.fromEntries(headers.map((h, i) => [h, jsonCell(r[i] ?? null)]))), null, 2));
    if (fmt === "csv")
        return withNotes(o.notes, toCsv([headers, ...rows]));
...
function resolveCol(headers, name) {
    ...
    if (!k)
        throw new UserError(`column ${JSON.stringify(name)} not found. Columns: ${headers.join(", ")}`);
    return k;
}
function aggValue(fn, vals) {
    const nonEmpty = vals.filter((v) => v !== null && String(v).trim() !== "");
    if (fn === "count")
        return nonEmpty.length;
    const nums = nonEmpty.map((v) => toNumber(v)).filter((n) => n !== null);
    if (nums.length === 0) {
        if (fn === "min" || fn === "max") {
...
function groupRecords(headers, recs, groupBy, aggs) {
    const gcols = groupBy.map((g) => resolveCol(headers, g));
    const specs = (aggs.length ? aggs : [{ col: "*", fn: "count", as: "count" }]).map((a) => {
        const isStar = String(a.col).trim() === "*";
        // v3 #12: every fn over "*" used to return the row count, so sum(*) silently answered
        // a different question than the one asked.
        if (isStar && a.fn !== "count")
            throw new UserError(`aggregate ${JSON.stringify(a.fn)} needs a column; "*" only works with count. Name the column to ${a.fn}.`);
    ...
    for (const c of gcols)
        taken.set(c.toLowerCase().trim(), `group column ${JSON.stringify(c)}`);
...
server.registerTool("sheet_query", {
...
}, guard(async ({ path, sheet, where, select, group_by, aggregate, sort, limit, as }) => {
    ...
    if ((group_by && group_by.length) || (aggregate && aggregate.length)) {
        const g = groupRecords(o.table.headers, recs, group_by ?? [], (aggregate ?? []));
        cols = g.headers;
        recs = g.rows;
        grouped = true;
    }
    if (sort) {
        ...
        if (!key)
            throw new UserError(`sort column ${JSON.stringify(sort.col)} not found. Columns: ${cols.join(", ")}`);
    ...
    if (select && select.length)
        headers = select.map((sname) => resolveCol(cols, sname));
    ...
    const fmt = as ?? "table";
    if (fmt === "json")
        return withNotes(o.notes, JSON.stringify(shown.map((r) => Object.fromEntries(headers.map((h) => [h, jsonCell(r[h] ?? null)]))), null, 2));
    if (fmt === "csv")
        return withNotes(o.notes, toCsv([headers, ...rows]));
...
server.registerTool("sheet_stats", {
...
}, guard(async ({ path, sheet, columns }) => {
    ...
    const wanted = columns && columns.length
        ? columns.map((s) => {
            ...
            if (!k)
                throw new UserError(`column ${JSON.stringify(s)} not found. Columns: ${o.table.headers.join(", ")}`);
            return k;
        })
        : o.table.headers;
    const out = wanted.map((h) => {
        ...
        if (nums.length && nums.length >= nonEmpty.length * 0.6) {
            ...
            for (const v of nonEmpty)
                freq.set(cellText(v), (freq.get(cellText(v)) ?? 0) + 1);
            ...
            if (lm) {
                res.min = cellText(nonEmpty[lens.indexOf(lm.min)]);
                res.max = cellText(nonEmpty[lens.indexOf(lm.max)]);
            }
        }
        return res;
    });
...
}, guard(async ({ path, text: needle, sheet }) => {
    const pro = gate.isPro();
    const wb = loadWorkbook(path, { maxRows: pro ? Infinity : FREE_MAX_ROWS });
...
server.registerTool("sheet_write", {
    title: "Write rows",
    description: "Call this tool to write rows to an xlsx, csv, tsv or json file; the extension of out_path picks the format. mode is new_file, append or overwrite. Other sheets are kept. Free: 500 rows per file.",
    inputSchema: {
...
}, guard(async ({ path, sheet, rows, mode, out_path }) => {
    const notes = [];
    let headers;
    let matrix;
    let target;
    ...
    if (mode === "new_file") {
        ...
        if (existsSync(target))
            throw new UserError(`${target} already exists. Pass out_path for a new name, or mode "overwrite" to replace it.`);
    }
    else {
        const o = open(path, sheet);
...
server.registerTool("sheet_add_column", {
    title: "Add a column",
    description: "Call this tool to add a computed column and save the result to a NEW file; the source is never modified unless out_path points at it. Returns the new file path, the row count and a preview of the first rows.",
    inputSchema: {
...
}, guard(async ({ path, sheet, name, formula, values, out_path, decimals }) => {
    if (!formula && !values)
        throw new UserError("give either formula or values");
    const o = open(path, sheet);
    const notes = [...o.notes];
    const headers = [...o.table.headers];
    if (headers.some((h) => h.toLowerCase().trim() === String(name).toLowerCase().trim()))
        throw new UserError(`column ${JSON.stringify(name)} already exists`);
    ...
    if (formula) {
        ...
        if (values.length !== recs.length)
            notes.push(`values has ${values.length} entries for ${recs.length} data rows; missing entries are blank.`);
...
}, guard(async ({ path, to, sheet, out_path }) => {
    const o = open(path, sheet);
    const notes = [...o.notes];
...
// ------------------------------------------------------------------ resource
server.registerResource("sheet", new ResourceTemplate("sheet://{path}", { list: undefined }), { title: "Spreadsheet overview", description: "sheet://<path> returns the sheet_info summary for a spreadsheet file", mimeType: "application/json" }, async (uri, vars) => {
...
server.registerResource("recent-sheets", "sheet://recent", {
