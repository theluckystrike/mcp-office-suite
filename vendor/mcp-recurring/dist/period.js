 ...
 */
export function addMonthsIso(iso, months, anchorDay, endOfMonth) {
    const [y, m, d] = parts(iso);
 ...
 */
function estimateStartK(rule, target) {
    const months = stepMonths(rule.every);
    if (months !== null) {
 ...
 */
export function occurrencesBetween(rule, to, from) {
    ...
    for (let k = startK; scanned < MAX_OCCURRENCES; k++, scanned++) {
        ...
        if (d < rule.start_date)
            continue; // anchoring moved the first one back
        if (d >= lower)
            out.push(d);
    }
    return out;
...
export function nextOccurrence(rule, after) {
    ...
    for (let k = startK; scanned < MAX_OCCURRENCES; k++, scanned++) {
        ...
        if (d < rule.start_date)
            continue;
        if (d > after)
            return rule.end_date && d > rule.end_date ? null : d;
    }
    return null;
}
