...
import { addDays, computeTotals, dataDir as invoiceDataDir, findClient, formatMoney, getBusiness, getClients, getInvoices, hasBusiness, invoiceLockPath, isoDate, nextNumber, renderInvoicePdf, setClients, setInvoices, } from "@theluckystrike/mcp-invoice/lib";
import { z } from "zod";
import { isKnownCurrency } from "./currency.js";
import { addMonthsIso, everyLabel, isIsoDate, nextOccurrence, occurrencesBetween, } from "./period.js";
import { dataDir, findSchedule, generatedKeys, getHistory, getSchedules, lockPath, setHistory, setSchedules, } from "./store.js";
...
const NO_BUSINESS_NOTE = "No business profile yet: generated PDFs show a placeholder issuer. " +
...
function isActive(s, asOf) {
    return s.status === "active" && !(s.end_date && s.end_date < asOf);
}
function activeCount(list, asOf) {
 ...
 */
function issueInvoice(s, period) {
    const biz = issuer();
    let client = findClient(s.client);
    if (!client) {
        const clients = getClients();
        const c = { id: randomBytes(4).toString("hex"), name: s.client.trim(), created: isoDate() };
        clients.push(c);
        setClients(clients);
        client = c;
    }
    const currency = (s.currency ?? biz.default_currency).toUpperCase();
    const totals = computeTotals(s.items, currency, 0, biz.default_tax_rate);
...
function dueRows(schedules, history, asOf, only) {
...
function scheduleTotalMinor(s, currency) {
    const biz = getBusiness();
    const t = computeTotals(s.items, currency, 0, biz.default_tax_rate);
 ...
 */
function scheduleAmounts(s, currency) {
    const biz = getBusiness();
    const t = computeTotals(s.items, currency, 0, biz.default_tax_rate);
...
/* ------------------------------------------------------------------- server */
const server = new McpServer({ name: "mcp-recurring", version: VERSION }, { capabilities: { tools: {}, resources: {}, prompts: {} } });
server.registerTool("schedule_create", {
    ...
    try {
        return await locked(() => {
            if (!isIsoDate(a.start_date))
                return fail(`start_date must be a real calendar date as YYYY-MM-DD, got "${a.start_date}".`);
            if (a.end_date !== undefined && !isIsoDate(a.end_date))
                return fail(`end_date must be a real calendar date as YYYY-MM-DD, got "${a.end_date}".`);
            if (a.end_date && a.end_date < a.start_date)
                return fail(`end_date ${a.end_date} is before start_date ${a.start_date}.`);
            if (a.currency && !isKnownCurrency(a.currency))
                return fail(`"${a.currency.toUpperCase()}" is not an ISO 4217 currency code. Use EUR, USD, GBP, PLN and so on.`);
            ...
            return ok(`Created schedule ${s.id} for ${s.client}, ${everyLabel(s.every)} from ${s.start_date}.\n\n` +
...
server.registerTool("schedule_list", {
    ...
    try {
        ...
        if (!list.length)
            return ok("No schedules yet. Create one with schedule_create.");
...
server.registerTool("schedule_get", {
    ...
    try {
        ...
        if (!s)
            return fail(`no schedule ${a.id}. List them with schedule_list.`);
...
server.registerTool("schedule_update", {
    ...
    try {
        return await locked(() => {
            ...
            if (!s)
                return fail(`no schedule ${a.id}. List them with schedule_list.`);
            if (a.start_date !== undefined && !isIsoDate(a.start_date))
                return fail(`start_date must be YYYY-MM-DD, got "${a.start_date}".`);
            if (a.end_date != null && !isIsoDate(a.end_date))
                return fail(`end_date must be YYYY-MM-DD, got "${a.end_date}".`);
            if (a.currency !== undefined && !isKnownCurrency(a.currency))
                return fail(`"${a.currency.toUpperCase()}" is not an ISO 4217 currency code. Use EUR, USD, GBP, PLN and so on.`);
            const pro = gate.isPro();
            let note = "";
            if (a.client !== undefined)
                s.client = a.client.trim();
            if (a.items !== undefined)
                s.items = a.items;
            if (a.currency !== undefined)
                s.currency = a.currency.toUpperCase();
            if (a.every !== undefined)
                s.every = a.every;
            if (a.start_date !== undefined)
                s.start_date = a.start_date;
            if (a.end_date !== undefined)
                s.end_date = a.end_date ?? undefined;
            if (a.due_days !== undefined)
                s.due_days = a.due_days;
            if (a.notes !== undefined)
                s.notes = a.notes;
            if (a.tax_note !== undefined)
                s.tax_note = a.tax_note.trim() || undefined;
            if (a.auto_generate !== undefined)
                s.auto_generate = a.auto_generate;
            if (a.anchor_day !== undefined || a.end_of_month !== undefined) {
                if (pro) {
                    if (a.anchor_day !== undefined)
                        s.anchor_day = a.anchor_day ?? undefined;
                    if (a.end_of_month !== undefined)
                        s.end_of_month = a.end_of_month;
                }
                else {
            ...
            if (s.end_date && s.end_date < s.start_date)
                return fail(`end_date ${s.end_date} is before start_date ${s.start_date}.`);
...
function setStatus(id, status) {
    return locked(() => {
        ...
        if (!s)
            return fail(`no schedule ${id}. List them with schedule_list.`);
...
server.registerTool("schedule_pause", {
    ...
    try {
        return await setStatus(a.id, "paused");
    }
    catch (e) {
        return fail(String(e.message ?? e));
    }
...
server.registerTool("schedule_resume", {
    ...
    try {
        return await setStatus(a.id, "active");
    }
    catch (e) {
        return fail(String(e.message ?? e));
    }
...
server.registerTool("schedule_skip", {
    ...
    try {
        return await locked(() => {
            ...
            if (!s)
                return fail(`no schedule ${a.id}. List them with schedule_list.`);
            if (!isIsoDate(a.period))
                return fail(`period must be a real calendar date as YYYY-MM-DD, got "${a.period}".`);
            const history = getHistory();
            const existing = history.find((h) => h.schedule_id === s.id && h.period === a.period);
            if (a.undo) {
                if (!existing)
                    return fail(`${s.client} has no record for ${a.period}, so there is nothing to undo.`);
                if (!existing.skipped)
                    return fail(`${a.period} was invoiced as ${existing.invoice_number}, not skipped. Delete that invoice in the invoice server if it was wrong.`);
...
server.registerTool("invoice_generate_due", {
    ...
    try {
        ...
        if (!isIsoDate(asOf))
            return fail(`as_of must be a real calendar date as YYYY-MM-DD, got "${asOf}".`);
        const created = [];
        const result = await lockedWithInvoice(() => {
            ...
            if (a.dry_run)
                return { rows, skipped, remaining, duplicates, dry: true };
            for (const r of rows) {
            ...
            setHistory(history);
            return { rows, skipped, remaining, duplicates, dry: false };
        });
        if (result.dry) {
            return json({
        ...
        const lines = created.map((c, i) => `${c.invoice.number}  ${c.schedule.client}  period ${c.period}  ${formatMoney(c.invoice.total_minor, c.invoice.currency)}  due ${c.invoice.due_date}  ${pdfs[i]}`);
        const totals = {};
        for (const c of created)
            totals[c.invoice.currency] = (totals[c.invoice.currency] ?? 0) + c.invoice.total_minor;
        return ok(`as_of ${asOf}: created ${created.length} invoice${created.length === 1 ? "" : "s"}, skipped ${result.skipped.length} already invoiced.\n\n` +
...
server.registerTool("schedule_history", {
    ...
    try {
        ...
        if (!rows.length)
            return ok(`No invoices generated for ${id} yet.`);
...
server.registerTool("forecast", {
    ...
    try {
        ...
        const totals = {};
        for (const [, per] of buckets)
            for (const [c, v] of Object.entries(per))
...
process.stderr.write(`mcp-recurring ready (${gate.isPro() ? "pro" : "free"}), schedules in ${dataDir()}, invoices in ${invoiceDataDir()}\n`);
