...
function zoneMidnight(y, m, d, zone) {
...
export function dayKey(isoStr, zone = homeZone()) {
 ...
 */
export function hhmm(isoStr, zone = homeZone()) {
    const d = new Date(isoStr);
    if (zone) {
...
export function localDayStart(daysBack = 0, zone = homeZone()) {
    if (zone) {
 ...
 */
export function wallClockInZone(s, zone = homeZone()) {
    if (!zone)
        return null;
