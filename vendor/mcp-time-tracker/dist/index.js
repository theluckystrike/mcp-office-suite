...
function parseTime(s, what) {
    ...
    if (inZone) {
        if (Number.isNaN(inZone.getTime()))
...
function parseRate(rate) {
    ...
    if (typeof rate === "number") {
        if (!Number.isFinite(rate) || rate < 0)
            throw new Error(`rate must be a non-negative number: ${rate}`);
        return { rate };
    }
    const txt = String(rate).trim();
    const n = parseAmount(txt);
    if (!Number.isFinite(n) || n < 0)
        throw new Error(`rate must be a non-negative number, got ${JSON.stringify(txt)}`);
...
function parseAmount(raw) {
    ...
    if (!t)
        throw new Error(`rate must contain a number, got ${JSON.stringify(raw)}`);
    if (/^\d+(\.\d+)?$/.test(t))
        return Number(t); // 90 / 90.50 / 1.2
    if (/^\d{1,3}(,\d{3})+(\.\d+)?$/.test(t))
        return Number(t.replace(/,/g, "")); // 1,200 / 1,200.50
    if (/^\d{1,3}(\.\d{3})+(,\d{1,2})?$/.test(t))
        return Number(t.replace(/\./g, "").replace(",", ".")); // 1.200,50
    if (/^\d+,\d{1,2}$/.test(t))
        return Number(t.replace(",", ".")); // 12,50
    throw new Error(`rate ${JSON.stringify(String(raw))} is ambiguous: write 1200 or "1,200.00" for one thousand two hundred, ` +
...
function currencyForEntry(db, e) {
    return e.currency ?? currencyFor(db, e.project);
}
function addAmount(m, currency, cents) {
    if (cents === 0 && !m.has(currency))
        return;
    m.set(currency, (m.get(currency) ?? 0) + cents);
}
function mergeAmounts(into, from) {
    for (const [c, v] of from)
        addAmount(into, c, v);
}
function nonZero(m) {
...
/* ------------------------------------------------------- project matching */
function knownProjects(db) {
    const s = new Set(Object.keys(db.projects));
    for (const e of db.entries)
        s.add(e.project);
    if (db.running)
        s.add(db.running.project);
    return [...s];
}
/**
 ...
 */
function resolveProject(db, input) {
    ...
    if (exact)
        return { kind: "use", project: exact, note: exact === given ? undefined : `Matched the existing project "${exact}".` };
    if (!q)
    ...
    if (near.length === 1)
        return { kind: "use", project: near[0], note: `Used the existing project "${near[0]}" (you said "${given}").` };
    if (near.length > 1)
        return { kind: "ambiguous", candidates: near.sort() };
    return { kind: "use", project: given };
}
function ambiguousText(given, candidates) {
...
function unbilled(entries, unbilledOnly) {
...
function resolveFilter(db, project) {
    if (!project)
        return { kind: "ok" };
    const r = resolveProject(db, project);
    if (r.kind === "ambiguous")
        return { kind: "ambiguous", text: ambiguousText(project, r.candidates) };
...
}, guard(async ({ project, task, tags, rate, currency }) => {
    return withFileLock(LOCK, async () => {
        const db = load();
        const r = resolveProject(db, project);
        if (r.kind === "ambiguous")
            return ok(ambiguousText(project, r.candidates));
        ...
        if (typeof parsed.rate === "number")
            lines.push(`Rate ${money(toCents(parsed.rate), cur ?? currencyFor(db, r.project))} per hour.`);
        if (r.note)
            lines.unshift(r.note);
        if (stopped)
...
server.registerTool("timer_status", {
    ...
    const w = { fromMs: todayStart, toMs: tomorrowStart - 1, clamped: false };
...
server.registerTool("entry_add", {
    ...
    return withFileLock(LOCK, async () => {
        ...
        if (r.kind === "ambiguous")
            return ok(ambiguousText(a.project, r.candidates));
...
server.registerTool("entry_edit", {
    ...
    return withFileLock(LOCK, async () => {
        ...
        if (a.rate !== undefined) {
            const parsed = parseRate(a.rate);
            if (typeof parsed.rate === "number")
                e.rateCents = toCents(parsed.rate);
            if (parsed.currency)
    ...
    return withFileLock(LOCK, async () => {
        ...
        if (a.apply_to_existing) {
            for (const e of projectEntries) {
                if (onlyMissing && typeof e.rateCents === "number")
                    continue;
                if (e.rateCents === m.rateCents && e.currency === m.currency)
                    continue;
        ...
        if (a.apply_to_existing) {
            ...
            lines.push(`${changed} of ${projectEntries.length} ${scope} re-rated (only_missing: ${onlyMissing}). ` +
...
function partsOf(e, by) {
    if (by === "day")
        return splitByDay(e);
    if (by === "project")
        return [{ key: e.project, seconds: e.seconds }];
    if (by === "task")
        return [{ key: e.task && e.task.trim() ? e.task : "(no task)", seconds: e.seconds }];
...
function aggregate(db, entries, by) {
    ...
    for (const e of entries) {
        ...
        for (const part of partsOf(e, by)) {
            const b = m.get(part.key) ?? { key: part.key, seconds: 0, billableSeconds: 0, amounts: new Map() };
            b.seconds += part.seconds;
            if (e.billable) {
                b.billableSeconds += part.seconds;
                addAmount(b.amounts, cur, amountCents(part.seconds, rc));
            }
            m.set(part.key, b);
        }
    }
...
const TAG_OVERLAP_NOTE = "Tag rows can overlap: an entry tagged twice appears in both rows. The total counts every entry once.";
server.registerTool("report", {
    ...
    if (fmt === "csv") {
        ...
        for (const b of buckets) {
            const parts = nonZero(b.amounts);
            if (parts.length === 0)
                lines.push([b.key, hours(b.seconds), hours(b.billableSeconds), "0.00", currencyFor(db, b.key)].map(csvCell).join(","));
            // one line per currency: mixed currencies must never be added together
            for (const [cur, cents] of parts)
                lines.push([b.key, hours(b.seconds), hours(b.billableSeconds), (cents / 100).toFixed(2), cur].map(csvCell).join(","));
        }
        for (const [cur, cents] of (totalParts.length ? totalParts : [[currency, 0]])) {
    ...
    if (buckets.length === 0)
        return ok(`No time tracked in that period.${note}`);
    const body = table([a.group_by, "hours", "billable h", "amount"], buckets.map(b => [b.key, hours(b.seconds), hours(b.billableSeconds), moneyOf(b.amounts)]));
...
server.registerTool("entry_mark_billed", {
    ...
    return withFileLock(LOCK, async () => {
        ...
        if (a.ids && a.ids.length) {
            ...
            if (missing.length)
                return err(`no entry with id ${missing.join(", ")}. Nothing was marked; run entry_list or invoice_summary for current ids.`);
        }
        else {
...
server.registerTool("invoice_summary", {
    ...
    for (const l of lines)
        addAmount(totals, l.currency, l.cents);
    ...
    return ok(`Invoice summary - ${r.project}\n` +
